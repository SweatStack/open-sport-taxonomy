...
- **Target** — the platform's native sport identifier. Shape is platform-specific: FIT uses `{sport, sub_sport}` integer pairs; Strava uses strings; HealthKit uses integers; Garmin Training API uses strings; Wahoo uses integers.
